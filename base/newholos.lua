...
---@param origin Vector Origin of this entity
---@param hitbox modelfun? Model hitbox, to make prop
---@return ModelInfo
function model.create(origin, hitbox)
    local mainModel
    local rig = model.rig(origin)
    if hitbox then
        mainModel = function()
            local holo = rig()
            if !holo then return end
        ...
        mainModel = function() return rig() end
    end
    return setmetatable(
        { origin = mainModel, bones = {} },
        ModelInfo
    )
end
...
---@overload fun(bone: string, mdl: modelfun)
---@return ModelInfo
...
function model.hitbox(tbl)
    local freeze = tbl.freeze or tbl[1] or false
    local mass = tbl.mass or tbl[2] or 30
    local mat = tbl.material or tbl[3] or ""
    return function()
        local pr = prop.createCustom(Vector(), Angle(), tbl, true)
...
function ModelInfo:create()
    ...
    for name, part in pairs(self.bones) do
        ...
        mdl.bones[name] = holo
        local parent = part.parent or originHolo
        local parentHolo = mdl.bones[parent]
        if !parentHolo then
